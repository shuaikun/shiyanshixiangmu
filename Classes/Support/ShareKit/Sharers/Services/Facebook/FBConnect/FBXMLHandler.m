...
- (id)topContainer {
  if (_stack.count < 2) {
    ...
    id object = _stack[_stack.count-2];
    if (object == [NSNull null]) {
      object = [NSMutableDictionary dictionary];
      _stack[_stack.count-2] = object;
    }
    return object;
  }
}
  ...
  for (NSInteger i = 0; i < _chars.length; ++i) {
    ...
    if (![whitespace characterIsMember:c]) {
      ...
      if ([topContainer isKindOfClass:[NSMutableArray class]]) {
        id object = @{self.topName: _chars};
        _stack[_stack.count-1] = object;
      } else {
        _stack[_stack.count-1] = _chars;
      }
      break;
    }
  }
...
- (void)parser:(NSXMLParser *)parser didStartElement:(NSString *)elementName
  ...
  id object = nil;
  if ([attributeDict[@"list"] isEqualToString:@"true"]) {
