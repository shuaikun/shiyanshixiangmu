...
- (id)parseXMLResponse:(NSData*)data error:(NSError**)error {
  ...
  if (handler.parseError) {
    ...
    NSInteger code = [errorDict[@"error_code"] intValue];
    NSDictionary* info = @{NSLocalizedDescriptionKey: errorDict[@"error_msg"],
      @"request_args": errorDict[@"request_args"]};
    if (error) {
...
- (void)call:(NSString*)method params:(NSDictionary*)params dataParam:(NSData*)dataParam {
  ...
  _params[@"api_key"] = _session.apiKey;
  _params[@"v"] = kAPIVersion;
  _params[@"format"] = kAPIFormat;

  if (![self isSpecialMethod]) {
    _params[@"session_key"] = _session.sessionKey;
    _params[@"call_id"] = [self generateCallId];
