...
//#error This file uses the classic ARC retain/release model; hints below... 
...
+ (NSString*)value
{
    return [ITTOpenUDID valueWithError:nil];
}
...
+ (NSString*)valueWithError:(NSError **)error
{
    ...
    for (int n=0; n<kOpenUDIDRedundancySlots; n++) {
        ...
        if (slotPB==nil) {
            ...
            NSDictionary* dict = [ITTOpenUDID _getDictFromPasteboard:slotPB];
    ...
    if (openUDID==nil) {        
        if (mostReliableOpenUDID==nil) {
            ...
            //
            openUDID = [ITTOpenUDID _generateFreshOpenUDID];
        } else {
    ...
    if (availableSlotPBid!=nil && (myRedundancySlotPBid==nil || [availableSlotPBid isEqualToString:myRedundancySlotPBid])) {
        ...
        if (openUDID && localDict)
            [ITTOpenUDID _setDict:localDict forPasteboard:slotPB];
    }
