...
{
    BOOL downloadViaWifiOnly = [[UserManager sharedUserManager] onlyShowImageOnWifi];
    if (downloadViaWifiOnly) {
        BOOL isWifiNow = [[[ITTNetworkTrafficManager sharedManager] networkType] isEqualToString:@"wifi"];
        return isWifiNow;
    }else
    {
        return YES;
...
- (void)donwloadWifiOnlySwitchOff:(NSNotification*)n
{
    [self loadImage:self.imageUrl placeHolder:self.placeHolder];
}
...
{
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(networkStateDidChanged:) name:kReachabilityChangedNotificationForITTImageView object:nil];
    
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(donwloadWifiOnlySwitchOff:) name:kOnlyShowImageOnWifiSwitchOff object:nil];
    ...
    [self setImageWithURL:URL placeholderImage:self.placeHolder options:SDWebImageProgressiveDownload progress:^(NSUInteger receivedSize, long long expectedSize) {
        if (expectedSize > 0) {
    ...
    completed:^(UIImage *image, NSError *error, SDImageCacheType cacheType)
     {
         ...
         if (strongSelf.delegate && [strongSelf.delegate respondsToSelector:@selector(imageViewDidLoaded:image:)]) {
             [strongSelf.delegate imageViewDidLoaded:strongSelf image:image];
         }
