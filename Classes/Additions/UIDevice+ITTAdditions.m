...
- (BOOL)is47InchScreen
{
    if ([[[UIDevice currentDevice] platformString] isEqualToString:IPHONE_6_NAMESTRING]){
        return YES;
    }
    return NO;
    //return (([[UIScreen mainScreen] bounds].size.height - 667) ? NO : YES);
 //   return (fabs((double)[ [ UIScreen mainScreen ] bounds ].size.height - 667.0) < DBL_EPSILON);
}

- (BOOL)is5InchScreen
{
    return (fabs((double)[ [ UIScreen mainScreen ] bounds ].size.height - 736.0) < DBL_EPSILON);
}
